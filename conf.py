...
# LOGO_URL = '/doc/images/Nuitka-Logo-Symbol.png'
...
DEPLOY_COMMANDS = {
    "default" : [
        "rsync -ravz --exclude=.git --chown www-data:git --chmod Dg+x output/ root@nuitka.net:/var/www/"
    ]
}
...
# Custom filter for HTML
def filterHTML(filename):
    import lxml.etree

    tree = lxml.etree.parse(filename, lxml.etree.HTMLParser())
    ...
    for node in tree.xpath("//div[@class='contents topic']"):
        node.attrib["class"] += " alert alert-primary float-md-right topic"

    # Make footer standout more
    for node in tree.xpath("//footer[@id='footer']"):
        node.attrib["class"] = "well navbar navbar-bottom"

    # Make user manual logo path not broken
    for node in tree.xpath("//img[@src='doc/images/Nuitka-Logo-Symbol.png']"):
        node.attrib["src"] = "/" + node.attrib["src"]
...
# (translatable)
CONTENT_FOOTER = """\
Nuitka web site, contents &copy; {date}     {license}\
<div data-social-share-privacy='true' class="float-md-right"></div>
"""
...
CONTENT_FOOTER = u'Contents &copy; %s <a href="mailto:Kay.Hayen@gmail.com">Kay Hayen</a>' % datetime.datetime.now().year

# Make sure we run proper year.
assert time.gmtime().tm_year >= 2019
...
# (translatable)
BODY_END = """
<script type="application/x-social-share-privacy-settings">{"path_prefix":"http://panzi.github.io/SocialSharePrivacy/","layout":"line","services":{"buffer":{"status":false},"delicious":{"status":false},"disqus":{"status":false},"facebook":{"status":false},"flattr":{"status":false},"gplus":{"status":false},"hackernews":{"status":false},"linkedin":{"status":false},"pinterest":{"status":false},"stumbleupon":{"status":false},"tumblr":{"status":false},"xing":{"status":false}}}</script>
...
TWITTER_CARD = {
    ...
    'image' : "http://nuitka.net/doc/images/Nuitka-Logo-Symbol.svg",
    'card': 'summary',          # Card type, you can also use 'summary_large_image',
    'site': '@kayhayen',        # twitter nick for the website
    'creator': '@kayhayen',     # Username for the content creator / author.
}
