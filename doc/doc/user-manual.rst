...
-  Python: Version 2.6, 2.7 or 3.3, 3.4, 3.5, 3.6, 3.7, 3.8, 3.9, 3.10,
   ...
   .. important::
      ...
      installed as well, but only during the compile time. That is
   ...
   .. admonition:: Binary filename suffix
      ...
      we can safely overwrite the binary without destroying the original
      source file.
...
.. note::
   ...
   Using cached paths will be relevant, e.g. when Windows Firewall
...
For the splash screen, you need to specify it as a PNG file, and then make
sure to disable the splash screen when your program is ready, e.g. has
completed the imports, prepared the window, connected to the database,
...
by providing your template, with a few of them built-in to Nuitka.
...
bundle on macOS. If you find something that is lacking, please let us know.
...
only may these be just the troublemakers, they also require more
memory, so get rid of that. Of course, you do have to check that your
program has all the needed dependencies before you attempt to compile,
or else the compiled program will equally not run.
...
approach works for Python code, but for compiled code, you should
...
a temporary location. The first one will be the original executable path,
...
The comments must be start lines, and indentation must be used to the
...
Nuitka will pick up ``ccache`` if it's found in system ``PATH``, and
...
involves using an external dependency walker to copy necessary
...
Currently, the following reports are included in Nuitka. You just use the
...
-  ``factory``
   ...
   testing only, and it's recommended to base any of your own development
   on. When updating it, you will very often get merge conflicts. Simply
...
will be. So this one should not be underestimated and a crucial step
of successful optimizations. Every option to produce a constant may
...
and is wrapped with a so-called "side_effects" expression, yet can
