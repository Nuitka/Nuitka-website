...
Compilation results on Windows for Python 3.4-3.7 share a problem with
...
initialize the run time, the encoding must be known, or dedicated code
...
Using these older versions for terminal programs is therefore highly
...
these older versions is not by itself a problem when there is no
terminal available.
...
If you know the encoding to use, you can work around the issue with
``PYTHONIOENCODING`` being set to the proper value other than ``utf-8``.
