...
.. code:: python
   ...
   try:
       ...
       # Service is being stopped, e.g. via shutdown or manual stop
       # you have a little time for cleanup before being hard-killed.
       ...
...
.. note::
   ...
   with the binary path set correctly, making updates relatively easy.
   Extra arguments passed after install become arguments passed to the
   service at run time.

   Every other configuration of the service could be added, but it would
   take work and somebody to fund that time. For now, this is mainly
...
.. note::
   ...
   :ref:`path specifications <path-specs>` for the allowed special
   monikers. Of course, absolute paths are allowed too.
